<?php
  //käivitame sessiooni
	...
	function signUp($name, $surname, $email, $gender, $birthDate, $password){
  	...
  	function addProfile($userid, $mydescription, $bgcolor, $txtcolor){
		$info = null;
	  	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	    $stmt = $conn->prepare("INSERT INTO vpuserprofiles (userid, description, bgcolor, txtcolor) VALUES (?,?,?,?)");

	    $stmt->bind_param("isss", $userid, $mydescription, $bgcolor, $txtcolor);
	   
	   	if($stmt->execute()) {
	   		$info = "õnnestus";
	   	} else {
	   		$info = "Mäda aPPI : " . $stmt->error;
	   	}
	...
	function checkProfilebyID($UserID){
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT bgcolor, txtcolor FROM vpusers3 WHERE userid=?");
